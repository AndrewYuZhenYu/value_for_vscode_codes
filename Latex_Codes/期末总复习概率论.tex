...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{有放回排列}：每次抽取一个，记录结果后放回，共抽取 $m$ 次。每次都有 $n$ 种选择，所以总共有 $n^m$ 种排列。
          \begin{itemize}[noitemsep]
              \item \textbf{举例}：从 1,2,3 三个数字中有放回地取 2 次，组成两位数，共有 $3^2=9$ 种（11,12,13,21,22,23,31,32,33）。
          \end{itemize}
    \item \textbf{无放回排列}：每次抽取一个，记录结果后不放回，共抽取 $m$ 次（$m \le n$）。
          \begin{itemize}[noitemsep]
...
\begin{align*}
     & \text{交换律：}\quad A\cup B = B\cup A,\quad A\cap B = B\cap A                                                     \\
     & \text{结合律：}\quad A\cup (B\cup C) = (A\cup B)\cup C,\quad A\cap (B\cap C) = (A\cap B)\cap C                     \\
     & \text{分配律：}\quad A\cup (B\cap C) = (A\cup B)\cap (A\cup C),\quad A\cap (B\cup C) = (A\cap B)\cup (A\cap C)     \\
     & \text{德摩根律（对偶律）：}\quad \overline{A\cup B} = \bar{A}\cap \bar{B},\quad \overline{A\cap B} = \bar{A}\cup \bar{B} \\
     & \text{吸收律：}\quad \text{若 }A\subset B\text{，则 }AB = A,\ A\cup B = B
\end{align*}

\begin{myquote}
    \textbf{速记口诀}：“长线变短线，符号变，开口方向反转”。即：长横线（整体取反）变成短横线（每个事件分别取反），$\cup$ 变成 $\cap$，$\cap$ 变成 $\cup$。
\end{myquote}
...
\begin{align*}
     & \text{性质①：}\quad P(\emptyset) = 0,\ P(\Omega) = 1,\ 0 \le P(A) \le 1.                                                  \\
     & \text{性质② 有限可加性：}\quad \text{若 }A_1, A_2, \dots, A_n\text{ 是两两互不相容的事件，则 }P(\bigcup_{i=1}^n A_i) = \sum_{i=1}^n P(A_i). \\
     & \text{性质③ 单调性：}\quad \text{若 }A \subset B\text{，则 }P(B-A) = P(B) - P(A)\text{，且 }P(A) \le P(B).                        \\
     & \text{性质④ 求逆公式（核心）：}\quad P(\bar{A}) = 1 - P(A).                                                                       \\
     & \text{性质⑤ 减法公式（核心）：}\quad P(A - B) = P(A\bar{B}) = P(A) - P(AB).                                                       \\
     & \text{性质⑥ 加法公式（核心，必考）：}\quad
    \begin{cases}
        P(A \cup B) = P(A) + P(B) - P(AB) \\[4pt]
        P(A \cup B \cup C) = P(A)+P(B)+P(C) - P(AB)-P(BC)-P(AC) + P(ABC)
    \end{cases}                                                           \\
     & \text{性质⑦ 次可加性：}\quad P(\bigcup_{n \ge 1} A_n) \le \sum_{n \ge 1} P(A_n)
\end{align*}
...
\begin{align*}
     ...
     & \text{推广（链式法则）：}\quad P(A_1 A_2 \dots A_n) = P(A_n | A_1 A_2 \dots A_{n-1}) P(A_{n-1} | A_1 A_2 \dots A_{n-2}) \dots P(A_2 | A_1) P(A_1)
\end{align*}
...
\subsubsection{3. 例题}
盒子中有编号为 $1,2,3,4$ 的 $4$ 张卡片，现从中任取一张。设 $A=\{取到1号或2号\}$，$B=\{取到1号或3号\}$，$C=\{取到1号或4号\}$。讨论 $A,B,C$ 的独立性。
...
\begin{itemize}[noitemsep]
    \item 使用容斥原理和德摩根律
    \item \textbf{全错排公式}：$\displaystyle D_n = n! \left(1 - \frac{1}{1!} + \frac{1}{2!} - \frac{1}{3!} + \dots + (-1)^n \frac{1}{n!}\right)$
    \item 当 $n$ 较大时，$D_n \approx \frac{n!}{e}$。
\end{itemize}
...
对于离散型随机变量，分布函数为：$\displaystyle F(x) = \sum_{x_i \le x} p_i$
...
\begin{align*}
    \text{概率密度：} & \quad f(x) = \begin{cases} \frac{1}{b-a}, & a < x < b \\ 0, & \text{其他} \end{cases}                 \\
    \text{分布函数：} & \quad F(x) = \begin{cases} 0, & x < a \\ \frac{x-a}{b-a}, & a \le x < b \\ 1, & x \ge b \end{cases} \\
    \text{期望：}   & \quad E(X) = \frac{a+b}{2}                                                                          \\
    \text{方差：}   & \quad D(X) = \frac{(b-a)^2}{12}
\end{align*}

\subsubsection{2. 指数分布 $X \sim E(\lambda)$（$\lambda > 0$）}
\begin{align*}
    \text{概率密度：} & \quad f(x) = \begin{cases} \lambda e^{-\lambda x}, & x > 0 \\ 0, & x \le 0 \end{cases} \\
    \text{分布函数：} & \quad F(x) = \begin{cases} 1 - e^{-\lambda x}, & x > 0 \\ 0, & x \le 0 \end{cases}     \\
    \text{期望：}   & \quad E(X) = \frac{1}{\lambda}                                                         \\
    \text{方差：}   & \quad D(X) = \frac{1}{\lambda^2}                                                       \\
    \text{无记忆性：} & \quad P(X > s+t \mid X > s) = P(X > t)
\end{align*}
\textbf{适用场景}：电子元件的使用寿命、生物体的寿命、服务时间等。

\subsubsection{3. 正态分布 $X \sim N(\mu, \sigma^2)$}
\begin{align*}
    \text{概率密度：} & \quad f(x) = \frac{1}{\sqrt{2\pi}\sigma} e^{-\frac{(x-\mu)^2}{2\sigma^2}},\quad -\infty < x < +\infty \\
    \text{参数：}   & \quad \mu\text{ 是均值（位置参数），}\sigma > 0\text{ 是标准差（尺度参数）}                                               \\
    \text{性质：}   & \quad f(x)\text{ 关于 }x = \mu\text{ 对称，在 }x = \mu\text{ 处取得最大值}
\end{align*}

\textbf{标准正态分布}：$\mu = 0$，$\sigma = 1$，记作 $N(0,1)$，其分布函数记为 $\Phi(x) = \int_{-\infty}^x \frac{1}{\sqrt{2\pi}} e^{-\frac{t^2}{2}} dt$
...
\subsubsection{5. 例题}
求 $\iint_D e^{x^2 + y^2} dxdy$，$D$ 为 $x$ 轴与 $y = \sqrt{1 - x^2}$ 围成的上半圆区域。

\textbf{解}：
被积函数为 $e^{x^2+y^2}$，适合用极坐标。
...
\begin{enumerate}[noitemsep]
    ...
    \item $F(x, y)$ 关于每个变量是单调不减函数
    \item $F(x, y)$ 分别是 $x$ 和 $y$ 的右连续函数
...
\begin{itemize}[noitemsep]
    \item $X$ 的边缘分布律：$\displaystyle p_{i\cdot} = P\{X = x_i\} = \sum_{j=1}^\infty p_{ij}$（表格中行求和）
    \item $Y$ 的边缘分布律：$\displaystyle p_{\cdot j} = P\{Y = y_j\} = \sum_{i=1}^\infty p_{ij}$（表格中列求和）
\end{itemize}
...
\begin{align*}
    X\text{ 的边缘概率密度：} & \quad f_X(x) = \int_{-\infty}^{+\infty} f(x, y) dy \\
    Y\text{ 的边缘概率密度：} & \quad f_Y(y) = \int_{-\infty}^{+\infty} f(x, y) dx
\end{align*}
...
\textbf{一维情形}：
\begin{itemize}[noitemsep]
    \item 离散型：$\displaystyle E[g(X)] = \sum_{k=1}^\infty g(x_k) p_k$
    \item 连续型：$\displaystyle E[g(X)] = \int_{-\infty}^{+\infty} g(x) f(x) dx$
\end{itemize}

\textbf{二维情形}：
\begin{itemize}[noitemsep]
    \item 离散型：$\displaystyle E[g(X, Y)] = \sum_{i=1}^\infty \sum_{j=1}^\infty g(x_i, y_j) p_{ij}$
    \item 连续型：$\displaystyle E[g(X, Y)] = \int_{-\infty}^{+\infty} \int_{-\infty}^{+\infty} g(x, y) f(x, y) dx dy$
\end{itemize}

\subsubsection{3. 数学期望的性质}
\begin{align*}
     & \text{1. } E(C) = C \quad\text{（常数期望等于自身）}              \\
     & \text{2. } E(aX + b) = aE(X) + b \quad\text{（线性性）}      \\
     & \text{3. } E(X \pm Y) = E(X) \pm E(Y) \quad\text{（可加性）} \\
     & \text{4. 若 } X, Y \text{ 独立，则 } E(XY) = E(X)E(Y)
\end{align*}
...
\begin{align*}
     & \text{1. } D(C) = 0 \quad\text{（常数方差为零）}                  \\
     & \text{2. } D(aX + b) = a^2 D(X)                           \\
     & \text{3. } D(X \pm Y) = D(X) + D(Y) \pm 2\text{Cov}(X, Y) \\
     & \text{4. 若 } X, Y \text{ 独立，则 } D(X \pm Y) = D(X) + D(Y)
\end{align*}
...
\begin{align*}
     & \text{1. } \text{Cov}(X, Y) = \text{Cov}(Y, X) \quad\text{（对称性）}                                \\
     & \text{2. } \text{Cov}(aX, bY) = ab\,\text{Cov}(X, Y)                                            \\
     & \text{3. } \text{Cov}(aX + b, Y) = a\,\text{Cov}(X, Y)                                          \\
     & \text{4. } \text{Cov}(X_1 + X_2, Y) = \text{Cov}(X_1, Y) + \text{Cov}(X_2, Y) \quad\text{（可加性）} \\
     & \text{5. } \text{Cov}(X, C) = 0                                                                 \\
     & \text{6. 若 } X, Y \text{ 独立，则 } \text{Cov}(X, Y) = 0 \quad\text{（反之不成立）}
\end{align*}
...
\textbf{解}：条件要求：不相关（或独立），方差存在且一致有界。
\begin{itemize}[noitemsep]
...
\subsubsection{5. 三个大数定律的对比}
\begin{center}
    \begin{tabular}{@{}p{3cm}p{6cm}p{4cm}@{}}
        \toprule
        \textbf{定律} & \textbf{条件}             & \textbf{结论}                                                  \\
        \midrule
...
\textbf{解}：
设 $X_i$ 为第 $i$ 箱重量，$E(X_i)=50$，$\sqrt{D(X_i)}=5$。$n$ 箱总重 $T_n = \sum_{i=1}^n X_i$，$E(T_n)=50n$，$\sqrt{D(T_n)}=5\sqrt{n}$。
...
\subsubsection{3. 棣莫弗-拉普拉斯定理（二项分布以正态分布为极限）}
...
\begin{align*}
     ...
     & \text{样本方差：}\quad S^2 = \frac{1}{n-1} \sum_{i=1}^n (X_i - \bar{X})^2 = \frac{1}{n-1} \left( \sum_{i=1}^n X_i^2 - n\bar{X}^2 \right) \\
     & \text{样本标准差：}\quad S = \sqrt{S^2}                                                                                                   \\
     & \text{样本的 }k\text{ 阶原点矩：}\quad A_k = \frac{1}{n} \sum_{i=1}^n X_i^k,\quad k = 1, 2, \dots                                           \\
     & \text{样本的 }k\text{ 阶中心矩：}\quad B_k = \frac{1}{n} \sum_{i=1}^n (X_i - \bar{X})^k,\quad k = 1, 2, \dots
\end{align*}
...
\begin{enumerate}[noitemsep]
    \item 写出似然函数：
          \begin{itemize}[noitemsep]
              \item 离散型：$\displaystyle L(\theta) = \prod_{i=1}^n P\{X = x_i\} = \prod_{i=1}^n p(x_i; \theta)$
              \item 连续型：$\displaystyle L(\theta) = \prod_{i=1}^n f(x_i; \theta)$
          \end{itemize}
...
\subsubsection{5. 例题}
设 $X$ 的概率密度为 $f(x; \theta) = \begin{cases} \theta x^{\theta-1}, & 0 < x < 1 \\ 0, & \text{其他} \end{cases}$，$\theta > 0$。求 $\theta$ 的矩估计和最大似然估计。
...
\begin{center}
    \begin{tabular}{@{}lll@{}}
        \toprule
        \textbf{待估参数} & \textbf{其他参数} & \textbf{$1-\alpha$ 置信区间}                                                                                                           \\
        \midrule
...
\begin{center}
    \begin{tabular}{@{}llllll@{}}
        \toprule
        \textbf{检验参数} & \textbf{情形}   & $\mathbf{H_0}$          & $\mathbf{H_1}$            & \textbf{检验统计量}                                   & \textbf{拒绝域}                                                                \\
        \midrule
        $\mu$         & $\sigma^2$ 已知 & $\mu = \mu_0$           & $\mu \ne \mu_0$           & $U = \frac{\bar{X} - \mu_0}{\sigma/\sqrt{n}}$    & $|U| \ge z_{\alpha/2}$                                                      \\
        $\mu$         & $\sigma^2$ 未知 & $\mu = \mu_0$           & $\mu \ne \mu_0$           & $T = \frac{\bar{X} - \mu_0}{S/\sqrt{n}}$         & $|T| \ge t_{\alpha/2}(n-1)$                                                 \\
        $\sigma^2$    & $\mu$ 已知      & $\sigma^2 = \sigma_0^2$ & $\sigma^2 \ne \sigma_0^2$ & $\chi^2 = \frac{\sum (X_i - \mu)^2}{\sigma_0^2}$ & $\chi^2 \le \chi^2_{1-\alpha/2}(n)$ 或 $\chi^2 \ge \chi^2_{\alpha/2}(n)$     \\
        $\sigma^2$    & $\mu$ 未知      & $\sigma^2 = \sigma_0^2$ & $\sigma^2 \ne \sigma_0^2$ & $\chi^2 = \frac{(n-1)S^2}{\sigma_0^2}$           & $\chi^2 \le \chi^2_{1-\alpha/2}(n-1)$ 或 $\chi^2 \ge \chi^2_{\alpha/2}(n-1)$ \\
        \bottomrule
    \end{tabular}
\end{center}
...
\begin{center}
    \begin{longtable}{@{}lllll@{}}
        \toprule
        \textbf{分布} & \textbf{记法}        & \textbf{参数}                       & \textbf{期望 $E(X)$}  & \textbf{方差 $D(X)$}    \\
        \midrule
        \endfirsthead
        \toprule
        \textbf{分布} & \textbf{记法}        & \textbf{参数}                       & \textbf{期望 $E(X)$}  & \textbf{方差 $D(X)$}    \\
        ...
        二项分布        & $B(n, p)$          & $n \ge 1,\ 0 < p < 1$             & $np$                & $np(1-p)$             \\
...
\begin{align*}
     & \text{求逆公式：}\quad P(\bar{A}) = 1 - P(A)                                                                       \\
     & \text{减法公式：}\quad P(A-B) = P(A) - P(AB)                                                                       \\
     & \text{加法公式（两事件）：}\quad P(A \cup B) = P(A) + P(B) - P(AB)                                                      \\
     & \text{加法公式（三事件）：}\quad P(A \cup B \cup C) = P(A)+P(B)+P(C) - P(AB)-P(BC)-P(AC) + P(ABC)                       \\
     & \text{条件概率：}\quad P(A|B) = \frac{P(AB)}{P(B)}                                                                 \\
     & \text{乘法公式：}\quad P(AB) = P(A)P(B|A)                                                                          \\
     & \text{全概率公式：}\quad P(B) = \sum_i P(A_i)P(B|A_i)                                                               \\
     & \text{贝叶斯公式：}\quad P(A_i|B) = \frac{P(A_i)P(B|A_i)}{\sum_j P(A_j)P(B|A_j)}                                    \\
     & \text{伯努利公式：}\quad P_n(k) = C_n^k p^k (1-p)^{n-k}                                                             \\
     & \text{德摩根律：}\quad \overline{A \cup B} = \bar{A} \cap \bar{B},\quad \overline{A \cap B} = \bar{A} \cup \bar{B}
\end{align*}

\subsubsection*{数字特征公式}
\begin{align*}
     & \text{期望定义（离散）：}\quad E(X) = \sum_i x_i p_i                                   \\
     & \text{期望定义（连续）：}\quad E(X) = \int_{-\infty}^{+\infty} x f(x) dx               \\
     & \text{方差定义：}\quad D(X) = E(X^2) - [E(X)]^2                                    \\
     & \text{协方差：}\quad \text{Cov}(X, Y) = E(XY) - E(X)E(Y)                          \\
     & \text{相关系数：}\quad \rho_{XY} = \frac{\text{Cov}(X, Y)}{\sqrt{D(X)}\sqrt{D(Y)}}
\end{align*}

\subsubsection*{中心极限定理}
\begin{align*}
     & \text{独立同分布 CLT：}\quad \frac{\sum X_i - n\mu}{\sqrt{n}\sigma} \approx N(0,1) \\
     & \text{二项分布 CLT：}\quad \frac{n_A - np}{\sqrt{np(1-p)}} \approx N(0,1)
\end{align*}
