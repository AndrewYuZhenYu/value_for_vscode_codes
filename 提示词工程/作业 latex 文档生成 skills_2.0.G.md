...
2. 页边距上下左右各 1.2cm。
3. 全局字号为小四号（`\zihao{-4}`）。
4. **核心排版技术**：采用“图层遮挡”技术。在页面底层（背景层）全局铺满行距 0.9cm 的灰色横线；在顶层（文字层）排版题目，并将题干放置于纯白背景框（`\whitebox`）中以遮挡横线。题干结束后的空白处自然漏出背景横线作为作答区。
5. 页码显示在整张纸水平居中、距离纸张底边 1.0cm 处，采用绝对定位，不占用正文空间，不改变页边距。
6. 严格按照用户给定的题目顺序排版，不得打乱题号顺序。
7. 支持用户指定哪些题目独占一整页、哪些页面放 2 道题、哪些页面放 3 道题。
...
以下导言区、背景横线循环、页码定位、`\whitebox` 定义**必须原样保留**，不得增删宏包，不得修改参数。
...
### 3.2 严格遵守 minipage 高度分配

每道题必须由 `minipage` 容器包裹，通过其高度来控制该题所占用的整体空间（即文字+留白作答区的高度）。
每页 3 题、每页 2 题、独占一页时，`minipage` 高度**必须严格按照下表执行**：

| 每页题数 | minipage 高度（强制） | 题目间距（强制）        |
| -------- | --------------------- | ----------------------- |
| 1 题     | `0.96\textheight`     | 无（末尾加 `\newpage`） |
| 2 题     | `0.48\textheight`     | `\vspace{0.3cm}`        |
| 3 题     | `0.31\textheight`     | `\vspace{0.2cm}`        |

**说明：** 题干内容必须放入 `\whitebox{...}` 中，多余的空间自然呈现底层横线作为作答区，不需要也不允许再使用手动画线命令。

### 3.3 题干过长时的处理

如果题干过长，导致背景白框向下延伸过大、作答区不足，或超出 `minipage` 预设高度，必须按以下顺序处理：

1. 优先调整题干内部的换行，减少不必要的空行；
2. 优先使用 `\\` 手动换行，避免段落间距过大；
3. **禁止**擅自把题目移到下一页；
4. **禁止**擅自修改 `minipage` 预设的高度比例。
...
以下模板中的题干、题号可以替换，但 **`minipage` 高度、`\vspace{}` 数值、`\whitebox` 包裹方式、`\newpage` 位置不得修改**。
...
如果最后一页题目不满，仍然按上述模板排版，**不要**为了填满页面而增加 `\vspace` 或修改 `minipage` 高度。
...
4. **禁止**使用 `\vfill`。
5. **禁止**修改导言区的图层定义代码及 `\whitebox` 定义。
6. **禁止**修改每页题数对应的 `minipage` 高度。
7. **禁止**在正文区使用旧版的画线命令（如 `\answerlines` 或 `\rule`），一切作答区均通过背景图层自然透出。
8. 题干文字**必须全部包裹在 `\whitebox{% ... }` 内**。
9. 所有题目必须按用户给定顺序排列。
10. 若用户指定“独占整页”，必须使用 `0.96\textheight`。
11. 若用户指定“每页两题”，必须使用 `0.48\textheight`，题目之间用 `\vspace{0.3cm}`。
12. 若用户指定“每页三题”，必须使用 `0.31\textheight`，题目之间用 `\vspace{0.2cm}`。
...
按每页三题排版，顺序为 26、29、3(2)，每道题 `0.31\textheight`，包含于 `\whitebox`，题目之间 `\vspace{0.2cm}`，不使用 `\vfill`。
...
第 1 页放 26；第 2 页放 29（独占，`0.96\textheight`）；第 3 页放 3(2) 及其后续题目。所有题干均包裹在 `\whitebox` 中。
...
- “图层遮挡”技术模板（含背景横线和页码定位）；
- 题干使用 `\whitebox` 遮挡背景横线；
- 每页 3 题（`0.31\textheight`，`\vspace{0.2cm}`）；
- 单题独占整页（`0.96\textheight`）；
- 每页 2 题（`0.48\textheight`，`\vspace{0.3cm}`）；
- **全程不使用 `\vfill`，无独立的画线代码**。

能力较弱的 AI 可直接在此基础上替换题目内容与题号，**不要改动导言区、图层定义、`\whitebox` 定义、`minipage` 高度以及 `\vspace` 数值**。
...
- [ ] 文档类是否为 `ctexart`？
- [ ] 页边距是否为 1.2cm？
- [ ] 导言区是否原封不动保留了 `tikzpicture` 背景层绘制与 `\whitebox` 定义？
- [ ] **是否彻查并完全排除了 `\vfill` 的使用？**
- [ ] **是否所有题干内容均包含在 `\whitebox{% ... }` 的作用域内？**
- [ ] 正文中是否不存在任何旧版的 `\answerlines` 指令？
- [ ] 每页三题是否使用 `0.31\textheight` 并使用 `\vspace{0.2cm}` 分隔？
- [ ] 每页两题是否使用 `0.48\textheight` 并使用 `\vspace{0.3cm}` 分隔？
- [ ] 独占一页是否使用 `0.96\textheight` 并在结束后加 `\newpage`？
- [ ] 题目顺序是否与用户要求一致？
- [ ] 数学公式是否规范？
- [ ] 代码是否可直接编译？
...
以上为作业 LaTeX 文档生成提示词（图层遮挡严格排版版），请严格遵循执行。
